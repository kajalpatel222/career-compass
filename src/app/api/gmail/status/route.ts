...
import { CALENDAR_SCOPE, hasGoogleScope, isGmailConfigured } from "@/lib/gmail";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ configured: true, connected: Boolean(user?.gmailConnection), calendarConnected: hasGoogleScope(user?.gmailConnection?.scope, CALENDAR_SCOPE), address: user?.gmailConnection?.gmailAddress || null, lastSyncedAt: user?.gmailConnection?.lastSyncedAt?.toISOString() || null, messageCount });
  } catch {
